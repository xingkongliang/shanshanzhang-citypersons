...
class caltech_to_coco():
    def __init__(self, image_set, set, devkit_path):
        ...
        self._vRng = [0.65, 1.0]          # acceptable obj occlusion levels
    ...
    def image_path_from_index(self, index):
        ...
        for ext in self._image_ext:
            iamge_name = index.split('_')[-1]
            set = index.split('_')[0]
            image_path = os.path.join(self._data_path, 'images', set, 'V000', iamge_name +  ext)
    ...
    def _load_image_set_index(self):
        ...
        """
        assert os.path.exists('{}/images'.format(self._devkit_path))
        assert os.path.exists('{}/annotations'.format(self._devkit_path))

        images_files = []
        image_index = []
        annotations_files = []
        for set in self._set:
            images_files_per_set = sorted(glob.glob('{}/images/{}/V000/*.png'.format(self._devkit_path, set)))

            image_index_per_set = list(map(lambda x: "{}_{}".format(set, x.split('/')[-1].split('.')[0]), images_files_per_set))
            annotations_files_per_set = list(map(lambda x: x.replace('images', 'annotations').replace('png', 'txt'), images_files_per_set))

            images_files.extend(images_files_per_set)
            image_index.extend(image_index_per_set)
            annotations_files.extend(annotations_files_per_set)
    ...
    def _load_caltech_annotation(self, image_id, index):
        """
        Load image and bounding boxes info from txt files of caltechPerson.
        """
        file_name = index.split('_')[-1]
        annotation_file = self._annotations_files[image_id]
    ...
    def caltech_to_coco_train(self, is_train=False, vis=False):
        ...
        for image_id, idx in enumerate(self.image_index):
            ...
            anno = self._load_caltech_annotation(image_id, idx)
...
if __name__ == '__main__':
    ...
    caltech_root = '/media/tianliang/DATA/DataSets/Pedestrian_Datasets/data-ETH'
    image_set = 'train'
    trainvaltest_set = ['set{:0>2}'.format(i) for i in range(0, 3)]

    caltech = caltech_to_coco(image_set, trainvaltest_set, caltech_root)
    train_json_name = "caltech_ETHo{}h{}_trainvaltest.json".format(str(int(caltech._vRng[0]*100)), str(caltech._hRng[0]))
